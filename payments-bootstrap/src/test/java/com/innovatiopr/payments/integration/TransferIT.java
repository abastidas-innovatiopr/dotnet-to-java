...
class TransferIT extends AbstractIntegrationTest {
    ...
    void openFundedAccounts() {
        CustomerId customer = customers.register("Ada", "Lovelace", "ada@example.com").orElseThrow();
        source = accounts.open(customer, "USD").orElseThrow();
        destination = accounts.open(customer, "USD").orElseThrow();
        payments.deposit(source, new BigDecimal("1000.00"), "USD", "Opening deposit").orElseThrow();
    }
    ...
    void writes_everything_in_one_transaction() {
        ...
        Result<UUID> result = payments.transfer(UUID.randomUUID().toString(), source, destination,
    ...
    void is_idempotent_across_retries() {
        ...
        Result<UUID> first = payments.transfer(key, source, destination, new BigDecimal("100.00"), "USD", "Rent");
        Result<UUID> second = payments.transfer(key, source, destination, new BigDecimal("100.00"), "USD", "Rent");
        Result<UUID> third = payments.transfer(key, source, destination, new BigDecimal("100.00"), "USD", "Rent");

        assertThat(first.orElseThrow()).isEqualTo(second.orElseThrow()).isEqualTo(third.orElseThrow());
    ...
    void rejects_a_reused_key_carrying_a_different_request() {
        String key = UUID.randomUUID().toString();
        payments.transfer(key, source, destination, new BigDecimal("100.00"), "USD", "Rent").orElseThrow();

        Result<UUID> conflicting = payments.transfer(key, source, destination,
                new BigDecimal("400.00"), "USD", "Something else");

        assertThat(conflicting.firstError().code()).isEqualTo("TRANSFER_IDEMPOTENCY_KEY_REUSED");
        assertThat(conflicting.firstError().type())
                .isEqualTo(com.innovatiopr.payments.shared.domain.ErrorType.CONFLICT);
        assertThat(balanceOf(source)).isEqualByComparingTo("900.00");
    }
    ...
    void every_ledger_transaction_balances() {
        for (int i = 1; i <= 10; i++) {
            payments.transfer(UUID.randomUUID().toString(), source, destination,
                    new BigDecimal(i + ".50"), "USD", "Transfer " + i).orElseThrow();
        }
    ...
    void balances_reconcile_against_the_ledger() {
        payments.transfer(UUID.randomUUID().toString(), source, destination,
                new BigDecimal("321.00"), "USD", "Reconcile me").orElseThrow();
        payments.withdraw(destination, new BigDecimal("21.00"), "USD", "Cash out").orElseThrow();
    ...
    void publishes_domain_events_through_the_outbox() {
        payments.transfer(UUID.randomUUID().toString(), source, destination,
                new BigDecimal("10.00"), "USD", "Event test").orElseThrow();
